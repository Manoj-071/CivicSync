...
export default function LoginScreen({ onNavigateToRegister, loginUser }) {
  ...
  // 🛠️ Google OAuth Simulation Engine
  const handleGoogleSignIn = async () => {
    try {
      const mockGoogleProfileData = {
        email: email.trim() || "officer.sandbox@civicsync.com",
        name: "Google Sandbox Officer",
      ...
      console.warn("Google endpoint connection failed. Simulating local context injection...");
      
      // 🧪 LOCAL TESTING BYPASS:
      // If your backend endpoint for Google auth isn't fully ready, uncomment the lines below
      // to test your Friend's Officer Dashboard tab transitions instantly:
      /*
      if (loginUser) {
        loginUser({
          email: "officer1@civicsync.com",
          name: "Field Officer Sandbox",
          role: "FIELD_OFFICER",
