...
import { fetchEscalatedTickets } from '../../../api/api';
...
export default function EscalatedTab() {
  ...
  const loadTickets = useCallback(async ({ isRefresh = false } = {}) => {
    ...
    try {
      ...
      setError('Unable to load escalated tickets. Pull down to retry.');
    } finally {
      setIsLoading(false);
  ...
  // Escalated tickets are usually already assigned, but a supervisor may still
  // want to reassign to a fresher/less-loaded officer — reuse the same modal.
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <Text style={styles.emptyText}>
            {error ? error : 'Nothing escalated right now — no SLA breaches or citizen rejections.'}
          </Text>
        }
      />
      <AssignOfficerModal
